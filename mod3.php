		 ...
		 $cantidad1=$_REQUEST['cantidad'];
            ...
            if($row = mysqli_fetch_array($result)) {
				...
				if($marca1==""  && $precio1==""  && $cantidad1==""){
                    ...
                    if($marca1!=""){
                        $result=mysqli_query($link," UPDATE `inventario` SET `marca`='$marca1' WHERE `inventario`.`Id_za`=$Id;");
                    }
                    if($precio1!=""){
                        if(is_numeric($precio1)) {
                        $result=mysqli_query($link," UPDATE `inventario` SET `precio`='$precio1' WHERE `inventario`.`Id_za`=$Id;");
                        }else{                   
                            $band=1;
                        }
                    }
                    if($cantidad1!=""){
                        if(is_numeric($cantidad1)) {
                        $result=mysqli_query($link," UPDATE `inventario` SET `cantidad`='$cantidad1' WHERE `inventario`.`Id_za`=$Id;");
                        }else{                      
                            $band=1;
                        }
                    }
